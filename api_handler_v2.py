...
def extract_flight_info(flight):
    ...
    # =========================================================================
    # [2026-02-07] 🦁 v24.19 最終交通整理：「ターミナル・出身地」による絶対判定
    # 便名(JL500等)や会社(IJ)によるT3強制判定をここで上書きし、正しい出口へ再分類します。
    # これにより「新千歳が4号に入らない」「国内線がT3へ消える」バグをロジックで解決します。
    # =========================================================================
    f_master_key = (str(final_origin) + str(dep.get('airport', '')) + str(origin_iata)).upper()
    
    # 1. 第2ターミナル(T2)の場合：3号か4号かを確定（北日本・国際線を4号へ）
    if term == "2":
        # 北日本ルート判定
        is_north = any(kw in f_master_key for kw in ["新千歳","函館","旭川","女満別","釧路","稚内","帯広","青森","秋田","三沢","CTS","HKD","AKJ","MMB","KUH","WKJ","OBO","AOJ","AXT","MSJ"])
        # T2到着の国際線判定（国内幹線コード以外を国際線とみなす）
        is_intl_t2 = (origin_iata != "UNK" and origin_iata != "" and origin_iata not in ["FUK","ITM","OKA","HIJ","KGS","MYJ","OKJ","KMQ","TKS","KUM","NGS","OIT","KMI","UKB","NGO","KIX","TOTTORI","YONAGO","IWAKUNI","TOYAMA","SHONAI","NOTO","HACHIJOJIMA","IZUMO"])
        
        if is_north or is_intl_t2:
            e_type = "4号(T2)"
            p_count = max(p_count, 240)
        else:
            e_type = "3号(T2)"

    # 2. 第1ターミナル(T1)の場合：1号か2号かを確定（T3への誤混入を完全に遮断）
    elif term == "1":
        # JAL500番台などがT3判定されていても、ここで国内出口(1or2号)へ引き戻す
        if airline_iata == "JL" and (f_num_str.startswith("5") or f_num_str.startswith("1")):
            e_type = "2号(T1北)"
        else:
            e_type = "1号(T1南)"

    # 3. 第3ターミナル(T3)の場合：出口は「国際(T3)」で固定
    elif term == "3":
        e_type = "国際(T3)"
