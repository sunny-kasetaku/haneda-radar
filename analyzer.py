...
def run_analyze():
    print("--- KASETACK Analyzer v4.9: 真・全方位スキャン版 ---")
    ...
    print(f"DEBUG: ファイルサイズ: {len(raw_content)} bytes")

    # --- 1. 【絶対律】生データの中身を直接確認 ---
    # JAL/ANAが含まれている場所の生データを200文字だけ露出
    for key in ["JAL", "ANA", "JL", "NH"]:
        pos = raw_content.find(key)
        if pos != -1:
            snippet = raw_content[max(0, pos-50):pos+150].replace('\n', ' ')
            print(f"✅ 生データ確認 [{key}]: ...{snippet}...")
            break

    # --- 2. 最小限の洗浄（ノイズ除去） ---
    # styleとscriptを消すと中身が消えるリスクがあるため、一旦「何もしない」で試します
    clean_content = raw_content 
    
    stands = {"P1": 0, "P2": 0, "P3": 0, "P4": 0, "P5": 0}
    flight_rows = []
    
    # 時刻検索：v4.6で成功した最もシンプルな形
    ...
    if len(time_matches) == 0:
        print("⚠️ 致命的な問題：時刻パターンが一つも見つかりません。")
