import re
import datetime
import json
import os
from config import CONFIG

def run_analyze():
    print("--- KASETACK Analyzer v7.5: 瓦礫撤去・要塞解体版 ---")
    if not os.path.exists(CONFIG["DATA_FILE"]):
        print("❌ エラー: raw_flight.txt がありません")
        return None

    jst = datetime.timezone(datetime.timedelta(hours=9))
    now = datetime.datetime.now(jst)
    
    with open(CONFIG["DATA_FILE"], "r", encoding="utf-8", errors='ignore') as f:
        raw_html = f.read()

    # --- 1. 【要塞解体】CSSとスクリプトの中身を完全に消し去る ---
    # これをやらないと、CSSの数値(10px等)を時刻と誤認します
    clean_html = re.sub(r'<(style|script)[^>]*>.*?</\1>', ' ', raw_html, flags=re.DOTALL | re.IGNORECASE)
    # その後、残ったHTMLタグを除去してテキスト化
    text_content = re.sub(r'<[^>]+>', ' ', clean_html)
    text_content = re.sub(r'\s+', ' ', text_content) # 余計な空白を1つに

    stands = {"P1": 0, "P2": 0, "P3": 0, "P4": 0, "P5": 0}
    flight_rows = []
    
    # 都市名翻訳マップ
    city_map = {
        "SAPPORO": "札幌", "CHITOSE": "札幌", "FUKUOKA": "福岡", 
        "OKINAWA": "那覇", "NAHA": "那覇", "OSAKA": "大阪", "ITAMI": "伊丹",
        "KANSAI": "関空", "HIROSHIMA": "広島", "KAGOSHIMA": "鹿児島"
    }
    
    # 時刻パターン: 22:50 または 10:50 PM (小数点はCSS誤認防止のため除外)
    time_pat = r'(\d{1,2})[:：](\d{2})\s*([APap][Mm])?'
    
    print("1. 瓦礫（CSS）を撤去し、深層のデータをスキャン中...")

    for m in re.finditer(time_pat, text_content):
        try:
            h = int(m.group(1))
            m_val = int(m.group(2))
            ampm = m.group(3)
            
            if not (0 <= h <= 23 and 0 <= m_val <= 59): continue
            
            if ampm:
                ampm = ampm.upper()
                if "PM" in ampm and h < 12: h += 12
                if "AM" in ampm and h == 12: h = 0
            
            # 日付またぎ補正
            f_t = now.replace(hour=h % 24, minute=m_val, second=0, microsecond=0)
            diff = (f_t - now).total_seconds() / 60
            if diff < -1000 and now.hour >= 21:
                f_t += datetime.timedelta(days=1)
                diff = (f_t - now).total_seconds() / 60

            # 判定ウィンドウ
            if not (CONFIG["WINDOW_PAST"] <= diff <= CONFIG["WINDOW_FUTURE"]):
                continue

            # 周辺を調査
            chunk = text_content[max(0, m.start()-200) : m.end()+400].upper()
            
            # キャリア特定
            carrier = "不明"
            if "JAL" in chunk or "JL" in chunk: carrier = "JAL"
            elif "ANA" in chunk or "NH" in chunk: carrier = "ANA"
            elif "SKY" in chunk or "BC" in chunk: carrier = "SKY"
            elif "AIR DO" in chunk or "ADO" in chunk: carrier = "ADO"
            elif "SOLASEED" in chunk or "SNA" in chunk: carrier = "SNA"
            elif "STARFLYER" in chunk or "SFJ" in chunk: carrier = "SFJ"
            
            if carrier == "不明": continue # キャリア不明はゴミとみなす

            # 都市特定
            origin = "不明"
            for eng, jap in city_map.items():
                if eng in chunk: origin = jap; break
            
            pax = 180 if any(x in chunk for x in ["777", "787", "350", "767"]) else 120
            s_key = "P5"
            if carrier == "JAL":
                s_key = "P2" if origin in CONFIG["NORTH_CITIES"] else "P1"
            elif carrier == "ANA": s_key = "P3"
            elif carrier == "SKY": s_key = "P1"
            elif carrier in ["ADO", "SNA", "SFJ"]: s_key = "P4"
            
            flight_rows.append({
                "time": f_t.strftime("%H:%M"), "flight": carrier, 
                "origin": origin, "pax": pax, "s_key": s_key
            })
        except: continue

    unique_rows = []
    seen = set()
    for r in flight_rows:
        id_str = f"{r['time']}-{r['flight']}-{r['origin']}"
        if id_str not in seen:
            seen.add(id_str); unique_rows.append(r)

    for r in unique_rows: stands[r['s_key']] += r['pax']
    
    result = {
        "stands": stands, "pool_preds": {k: max(0, 100 - int(v/10)) for k, v in stands.items()},
        "total_pax": sum(stands.values()), "rows": unique_rows, "update_time": now.strftime("%H:%M")
    }
    
    with open(CONFIG["RESULT_JSON"], "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2)

    print(f"2. 解析完了。有効便数: {len(unique_rows)} / 総需要: {result['total_pax']}人")
    return result
