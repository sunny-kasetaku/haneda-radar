...
def run_render(password):
    ...
    for f in raw_flights:
        if "欠航" in f.get("status", "") or "Cancelled" in f.get("status", ""): continue
    ...
    if total_pax >= 800: r, c, sym, st = "S", "#FFD700", "🌈", "【最高】 需要爆発"
    elif total_pax >= 400: r, c, sym, st = "A", "#FF6B00", "🔥", "【推奨】 需要過多"
    elif total_pax >= 100: r, c, sym, st = "B", "#00FF00", "✅", "【待機】 需要あり"
    ...
    <body>
        <div id="main-content">
            ...
            <div style="display: flex; justify-content: space-around; font-size: 12px; color: #999; margin-bottom: 20px; background: #111; padding: 10px; border-radius: 10px;">
