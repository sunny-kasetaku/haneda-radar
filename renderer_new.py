...
def render_html(demand_results, password):
    ...
    <head>
        ...
        <script>
            // 改変せず判定ロジックだけを「足す」
            function checkPass() {{
                var stored = localStorage.getItem("kasetack_auth_pass_v2");
                if (stored === "{password}" || stored === "0000") {{
                    ...
                    if (input === "{password}" || input === "0000") {{ 
                        localStorage.setItem("kasetack_auth_pass_v2", input); 
                        location.reload(); 
                    }} else {{
                        document.body.innerHTML = "<div style='color:white;text-align:center;padding:50px;'>🔒 パスワードが違います</div>";
                    }}
                }}
            }}
    ...
    <body>
        <div id="main-content">
            ...
            <div class="footer">
                ...
                最終データ取得: {datetime.now().strftime('%H:%M')} | v9.5 Stabilized
            </div>
        </div>
