...
def render_html(demand_results, password):
    ...
    # ---------------------------------------------------------
    # 1. ランク判定 (修正: 文字列が含まれていても数値のみ抽出して計算)
    # ---------------------------------------------------------
    def to_int(v):
    ...
    for i, name in enumerate(target_keys):
        ...
        # 表示は受け取った文字列(例: "1500人")をそのまま使う
        disp_val = demand_results.get(name, "0人")
        
        cards_html += f"""
        <div class="t-card {cls}" {style}>
            {badge}
            <div style="color:#999;font-size:12px;">{name}</div>
            <div class="t-num">{disp_val}</div>
        </div>
        """
    ...
    <body>
        <div id="main-content">
            ...
            <div class="cam-box">
                ...
                <div class="disclaimer">※本システムは航空機のデータのみに基づいています。</div>
            </div>
            <button class="update-btn" onclick="location.reload(true)">最新情報に更新</button>
            <div class="footer">
                最終データ取得: {datetime.now().strftime('%H:%M')} | v9.6 Final
            </div>
        </div>
